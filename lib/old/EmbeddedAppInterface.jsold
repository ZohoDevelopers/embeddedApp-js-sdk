...
ZOHO.embeddedApp=(function(){
	...
	return{
		...
		handleMsg	:	function(message)
		{
			...
			{
				var fns = ZOHO.embeddedApp.events[eventName];
...
window.addEventListener("message", function receiveMessage(event)
{
	if(event)
	{
		...
		if(!ZOHO.embeddedApp.isAllowedOrigin(origin)){
			return;
		}
		/*
		...
		if( "object" === typeof(data) && data["MESSAGE.TO"] === "EMBEDDED_APP")
		{
			...
			ZOHO.embeddedApp.handlePostMessage(message);
		}

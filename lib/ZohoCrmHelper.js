            ...
            init: function() {
            	if(!isInitTriggered)
            	{
                    ...
                    appSDK.OnLoad(function() {
                        appSDK.getContext().Event.Trigger('ZDK_EVENT', {action : 'get_zdk_url', sdkVersion : '1.5'}, true)
                            .then(function (url) {
                                self._zdksdk = appSDK;
            ...
            return {
                ...
                CONFIG: {
                    ...
                    *
                     ...
                     * @example
                     * ZOHO.CRM.CONFIG.getUserPreference().then(function(data){
                     * 	console.log(data);
                     * });
                     * 
                     * //prints 
                ...
                API: {
                     ...
                     * @param {String} [config.approved=true] - To get approved record
...
(() => {
    ...
    function newRequestPromise(data) {
        ...
        if(!self.isInitTriggered){
            console.warn('ZRC tried to contact parent before init being called');
            return;
        }
			...
			return res
				...
				.catch((err) => {
                    ...
                    if(err.error && err.error.errorType && typeof err.error.errorType === "string"){
                        ...
                        const actualError = err.error;
                        switch(actualError.errorType){
                            case "ConnectionError":
                                err.error = new ConnectionError(actualError.errorMessage ? actualError.errorMessage : "Connection request failed");
                                break;
                            case "ZrcValidationError":
                                err.error = new ZrcValidationError(actualError.errorMessage);
                                break;
                            case "ZrcError":
                                err.error = new ZrcError(actualError.errorMessage);
                                break;
                            case "ApiError":
                                err.error = new ApiError(actualError.errorMessage);
                                break;
                            default:
                                err.error = new ZrcError(actualError.errorMessage);
                                break;
	...
	class ZRC {
        ...
        async #restCall(method, path, body, requestConfig) {
            ...
            const isBaseUrlSameAsAppOrigin = requestConfig.baseUrl
                ? new URL(requestConfig.baseUrl).origin ===
                  (await this.#PageUrlResolver()).origin
                : true;
            const isValidCrmPath = ZrcValidations.isRelativePath(path)
                ? path.startsWith("/crm/")
                : new URL(path).origin ===
                  (await this.#PageUrlResolver()).origin;

            const isCrmRequest = isValidCrmPath && isBaseUrlSameAsAppOrigin;
            const isConnection = requestConfig.connection ? true : false;

            // add credentials only if user does not provide the baseUrl property
            if (isCrmRequest || isConnection) {
                /**
                 * @type {{ headers: Record<string, any> }}
                 */
                // const credentials = await this.#ZDKCredentialResolver();
                requestConfig.headers = {
...
var ZDK = {
    Client: (function () {
        var zdksdk = self._getAppSDK();

        function zdkRequestPromise(data) {
            data['sdkVersion'] = '1.5';
            return zdksdk.getContext().Event.Trigger("ZDK_EVENT", data, true);
        }
        return {
            sendResponse: function (request_id, data) {
                return zdkRequestPromise({ // no i18n
            ...
            showConfirmation: function (message, accept_message, reject_message) {
            ...
            getInput: function (options, heading, accept_message, reject_message) {
                if (options) {
                    if (!Array.isArray(options)) {
                        ...
                        options.forEach(option => {
                            ...
                            if (['picklist', 'multiselectpicklist'].includes(type) && !list_options) {
            ...
            openPopup: function (config, data) {
                const {
                    id,
                    type,
                    api_name,
